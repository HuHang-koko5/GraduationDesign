...
# return: i(int)
def SI_content(doc, tag):
    # ------------init------------------------------
    si_dict = load_SI_dict()
        ...
        if tag[i] not in finaltag:
            ...
            if tag[i] not in atag:  # follow-only type
                ...
                if i + 1 < itemNum:  # has follow
                    if i + 1 < itemNum:
                        follow_value *= get_value(doc[i], si_dict)
                    #   print("followed", follow_value)
            else:  # both-side type
                # print("both-side type:",doc[i],tag[i])
                if get_close(i, tag, finaltag) or follow_value != 1:  # follow
                    follow_value = get_value(doc[i], si_dict)
                #  print("followed", follow_value)
                else:  # forward
                    if len(index_group):
                        value_group[len(index_group) - 1] *= get_value(doc[i], si_dict)
                #   print("forwarded", get_value(doc[i], si_dict))
        else:  # finaltag
            index_group.append(i)
            word_value = follow_value * get_value(doc[i], si_dict)
            follow_value = 1
